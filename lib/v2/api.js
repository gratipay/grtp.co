(function(_) {
    ...
    Gratipay.widgets = {
        ...
        apply: function(element, cb) {
            var projectslug = element.getAttribute('data-gratipay-projectslug');

            function set(className, property, value, attribute, append) {
                var elements = element.querySelectorAll('.gratipay-' + className);
            ...
            // Set up links and project slug before requesting public.json
            link('profile-link', gratipayURI + encodeURIComponent(projectslug) + '/');
            link('link', gratipayURI);
            text('slug', projectslug);

            Gratipay.users.getPublic(projectslug, function(data) {
                ...
                text('receiving', (data.receiving) ? '$' + parseFloat(data.receiving).toFixed(2) : '$0.00');
                text('slug', data.slug);
        ...
        load: function(element) {
            ...
            Gratipay.widgets.getWidget(widgetType, function(widget) {
                ...
                widget.setAttribute('data-gratipay-projectslug', projectslug);
